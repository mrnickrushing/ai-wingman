...
export function SessionTranscriptExplorer({ title, transcriptText, onBookmark, onShare }: Props) {
  ...
  const segments = useMemo(() => splitTranscript(transcriptText), [transcriptText]);
  const query = search.trim().toLowerCase();

  const matches = useMemo(() => {
    if (!query) return segments;
    return segments.filter((segment) => segment.toLowerCase().includes(query));
  }, [query, segments]);
  ...
  const selectedExcerpt = matches[selectedIndex] ?? matches[0] ?? segments[0] ?? '';
  ...
  return (
    <View style={s.card}>
      ...
      <View style={s.controls}>
        ...
        <Pressable
          onPress={() => onBookmark(selectedExcerpt || transcriptText || 'No transcript available.')}
          style={s.controlBtn}
        >
        ...
        <Pressable
          onPress={() => {
            const text = selectedExcerpt || transcriptText || 'No transcript available.';
            if (onShare) {
              onShare(text);
              return;
            }
      ...
      <View style={s.excerptCard}>
        ...
        <Text style={s.excerptText}>{selectedExcerpt ? highlight(selectedExcerpt, query) : 'No transcript text captured for this session.'}</Text>
      </View>

      {query && hasMatches ? (
        <View style={s.matchList}>
          {matches.slice(0, 5).map((segment, index) => (
            <Pressable
              key={`${index}-${segment.slice(0, 20)}`}
              onPress={() => setSelectedIndex(index)}
              style={[s.matchItem, index === selectedIndex && s.matchItemActive]}
            >
              <Text style={s.matchIndex}>{index + 1}</Text>
              <Text style={s.matchText} numberOfLines={3}>{highlight(segment, query)}</Text>
            </Pressable>
          ))}
        </View>
      ) : null}
