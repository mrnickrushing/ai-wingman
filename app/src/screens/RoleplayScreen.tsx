...
export function RoleplayScreen({ onBack, mode }: Props) {
  ...
  const roleplayContext = useMemo(() => {
    return [preset.context, memoryContext].filter(Boolean).join('\n');
  }, [memoryContext, preset.context]);

  const conversationTurns = useMemo<Turn[]>(() => {
    const userTurns = transcript.map((item) => ({
      id: `u-${item.id}`,
      speaker: 'You' as const,
      text: item.text,
      timestamp: item.timestamp,
    }));
    const assistantTurns = coachingHistory.map((item) => ({
      id: `c-${item.id}`,
      speaker: 'Claude' as const,
      text: item.text,
      timestamp: item.timestamp,
    }));
    return [...userTurns, ...assistantTurns].sort((a, b) => a.timestamp - b.timestamp || a.id.localeCompare(b.id));
  }, [coachingHistory, transcript]);
  ...
  const startRoleplay = async () => {
    ...
    try {
      ...
      Alert.alert('Roleplay failed', err instanceof Error ? err.message : 'Could not start voice roleplay.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
          ...
          {currentCoaching ? (
            <View style={s.tipCard}>
              <Text style={s.sectionLabel}>CURRENT REPLY</Text>
              <Text style={s.tipText}>{currentCoaching}</Text>
              {isWingmanSpeaking ? <Text style={s.tipMeta}>Speaking now</Text> : null}
            </View>
          ) : null}
...
const s = StyleSheet.create({
  ...
  tipText: { color: '#e2e8f0', fontSize: 14, lineHeight: 20 },
  tipMeta: { color: '#94a3b8', fontSize: 11, fontWeight: '800' },
  errorCard: {
